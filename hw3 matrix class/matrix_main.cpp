...
void StudentTest(){
	...
	cout << "Matrix m2(3,4,5) :" << endl << m2;
	cout << "Matrix m3(m2) :" << endl << m3;

	//get() set()
	double getvalue;
	cout << "m2.get(0,0,variable) variable: " << m2.get(1, 1, getvalue);
	cout<< " " << getvalue;
	cout << endl  << "m1.get(1,1,variable) : " << m1.get(0, 0, getvalue) << endl;
	cout << "m2.set(1,2,3) : " << m2.set(1, 2, 3)<<endl;
	cout << "m2.set(3,1,2) : " << m2.set(3, 1, 2) << endl;
	cout << "m2.set(2,3,1) : " << m2.set(2, 3, 1) << endl;
	cout << "m1.set(1,2,3) : " << m1.set(1, 2, 3) << endl;
	cout << "m2:" << endl << m2;


	//get_row() get_col()
	double * a = m2.get_col(2);
	cout << "m2.get_col(2): " << endl;
	for (unsigned int i = 0; i < m2.num_rows(); i++) 
		cout <<a[i] <<" "<< endl;
	cout << "m2.get_row(2): " << endl;
	delete[] a;
	a = m2.get_row(2);
	for (unsigned int i = 0; i < m2.num_cols(); i++) 
		cout << a[i] << " " << endl;
	delete [] a;
	
	//add() subtract() multiply_by_coefficient()
	cout << "m2:" << endl << m2;
	cout << "m3:" << endl << m3;
	cout << "m3.add(m2) m3:" << m3.add(m2) << endl;
	cout<<m3;
	cout << "m1.add(m2):"<< m1.add(m2) << endl ;
	cout << "m2.subtract(m3) m2:" << m2.subtract(m3) << endl;
	cout << m2;
	m2.multiply_by_coefficient(-1);
	cout << "m2.multiply_by_coefficient(-1):" << endl<<m2;

	//operator=
	cout << "m1:" << endl << m1;
	m1 = m2;
	cout << "m1=m2 m1:" << endl << m1;

	//swap_row() transpose()
	cout << "m3:" << endl << m3;
	m3.swap_row(1, 2);
	cout << "m3.swap_row(1, 2) m3:" << endl << m3;
	m3.transpose();
	cout << "m3.transpose() m3:" << endl << m3;

	

	//quarter()
	Matrix * q = m3.quarter();
	cout << "m3.quarter() :" << endl;
	for (unsigned int i = 0; i < 4; i++) cout << *(q+i);
	delete[] q;
...
void ExtraCreditTest(){
	...
	cout << "m3:" << endl << m3;
	m3.resize(5, 5, 0);
	cout << "m3.resize(5,5,0) m3:" << endl << m3;
	m3.resize(5, 3, 0);
	cout << "m3.resize(5,3,0) m3:" << endl << m3;
	m3.resize(0, 0, 0);
	cout << "m3.resize(0,0,0) m3:" << endl << m3;
}
